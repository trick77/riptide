...
WITH bot_identities AS (
  -- Accounts a sender declared as automation, learned from the stream rather
  -- than from config: noergler reports the account it comments under and that
  -- it is a bot. `is_automated` is decided at ingest, so rows written before
  -- an identity was declared are filtered here instead.
  SELECT DISTINCT lower(reviewer_handle) AS handle
  FROM noergler_events
  WHERE reviewer_handle IS NOT NULL AND reviewer_is_bot
),
pickup_start AS (
  SELECT
    repo_full_name,

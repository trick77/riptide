...
- `completed` rows always carry `commit_sha` (required) and join the
  other event tables on the universal `commit_sha` join key
  (AGENTS.md "Commit SHA is the universal join key").
- `feedback` rows carry `commit_sha` *optionally* — when noergler knows
  which commit the finding was raised on, it emits it; if absent,
  reviewer-precision aggregates fall back to team / repo / week.
- `pr_key` and `actor` are lowercased for stable joins against future
  Bitbucket / directory rollups.
"""

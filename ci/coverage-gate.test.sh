#!/usr/bin/env bash
# ci/coverage-gate.test.sh — self-test for coverage-gate.sh
set -uo pipefail
...
<coverage>
  <packages>
    <package name="github.com/trick77/riptide/internal/review">
      <classes>
    ...
    <package name="github.com/trick77/riptide/cmd/riptide">
      <classes>
        <class name="main" filename="cmd/riptide/main.go">
...
COVERAGE_FLOORS="$TMP/floors-under" COVERAGE_FILE="$TMP/backend.xml" \
  ./ci/coverage-gate.sh backend >/dev/null 2>&1
...
out=$(COVERAGE_FLOORS="$TMP/floors-under" COVERAGE_FILE="$TMP/backend.xml" \
  ./ci/coverage-gate.sh backend 2>&1)
case "$out" in

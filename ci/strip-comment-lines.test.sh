#!/usr/bin/env bash
# ci/strip-comment-lines.test.sh — self-test for strip-comment-lines.go
#
...
kept() { # kept <line-numbers...> -> the line numbers that survived, comma-joined
  report "$@" | go run ./ci/strip-comment-lines.go "$TMP" 2>/dev/null |
...
check "keeps a string that contains //" "4" "$(
  report 4 | sed 's|src/sample.go|src/urls.go|' |
    go run ./ci/strip-comment-lines.go "$TMP" 2>/dev/null |
    sed -nE 's/.*<line number="([0-9]+)".*/\1/p' | paste -sd, -
)"
